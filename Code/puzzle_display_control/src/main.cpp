...
void setup()
{
  ...
  tft.setFont(&FreeSerifBoldItalic9pt7b);
    ...
    letter.x = 400;
    letter.y = 240;
  calibrateTSPoint(&calibrated, &letter, &_tsMatrix );
  Serial.println(calibrated.x);
  Serial.println(calibrated.y);
  char mychar = 'A';
  tft.drawChar(letter.x,letter.y, mychar, RA8875_BLUE, RA8875_WHITE, 7);
}
...
void loop()
{
  ...
  tsPoint_t midpoint;
  midpoint.x = letter.x + 25;
  midpoint.y = letter.y + 25;
  ...
  calibrateTSPoint(&calibrated, &raw, &_tsMatrix );
    ...
    if(abs(calibrated.x - midpoint.x) < 40 && abs(calibrated.y - midpoint.y) < 40)
    {
        ...
        tft.drawChar(letter.x,letter.y, 'A', RA8875_RED, RA8875_WHITE, 7);
    }
    else
    { 
        Serial.println("Failed: ");
        tft.drawChar(letter.x,letter.y, 'A', RA8875_YELLOW, RA8875_WHITE, 7);
    }
